    ...
    async def handle_message(self, message_in: Message, channel_pool: aio_pika.pool.Pool):
        ...
        try:
            async for message_out in self.do_work(message_in):
                raise Exception('ex')
                message = aio_pika.Message(body=encodes(message_out).encode('utf-8'), timestamp = dt)
            ...
            message_in.scope.metadata['ts'] = dt
            message = aio_pika.Message(body=jsons.dumps(message_in).encode('utf-8'), timestamp = dt)
